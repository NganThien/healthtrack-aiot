...
> **Cập nhật kiến trúc:** service này giờ đóng vai trò **MQTT → Supabase bridge**.
> Postgres không còn chạy local nữa — set `DATABASE_URL` trong `.env` trỏ tới
> connection string (Session pooler) của project Supabase Cloud, xem
> `.env.example` và [supabase/migrations/0001_init.sql](../supabase/migrations/0001_init.sql).
> Các biến `DB_HOST/DB_PORT/...` bên dưới chỉ còn là fallback cho Postgres local
> khi dev offline không có `DATABASE_URL`. Có thể chạy service này bằng Docker
> qua `docker-compose.yml` ở thư mục gốc thay vì `npm run dev`.
