...
// Schema (bảng, index, RLS) được quản lý bởi Supabase migration
// (supabase/migrations/0001_init.sql), bridge không tự tạo bảng nữa.
// Chỉ cần đảm bảo device_id tồn tại trong bảng `devices` trước khi insert,
// vì sensor_data/emg_data có FK device_id -> devices(id).
const knownDeviceIds = new Set();

async function ensureDevice(deviceId) {
  if (!deviceId || knownDeviceIds.has(deviceId)) return;
  await pool.query(
...
async function startMqttSubscriber() {
  ...
  client.on('message', async (topic, message) => {
    try {
      ...
      await ensureDevice(data.device_id);
